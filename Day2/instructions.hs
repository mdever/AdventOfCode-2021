...
data Position = Position { depth :: Int, forward :: Int } deriving (Show)
startingPosition = Position 0 0

processInstructions :: [Instruction] -> Position
processInstructions = foldl (\p i -> case i of
                               (Up amount)      -> Position ((depth p) - amount) (forward p)
                               (Down amount)    -> Position ((depth p) + amount) (forward p)
                               (Forward amount) -> Position (depth p) ((forward p) + amount)
                               otherwise        -> p
                               ) startingPosition
